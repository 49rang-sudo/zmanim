...
export async function markOrderPaid(
  ...
  if (!reservation.ok) {
    // התשלום כבר בוצע ולא ניתן "לבטל" אותו כאן — אבל אחת (או יותר)
    // מהמהדורות שנרכשו כבר פגה/התמלאה בינתיים. חובה לסמן לטיפול
    // ידני (החזר חלקי/מהדורה חלופית) ולא להשאיר את זה בשקט.
    const alert = `⚠ שולם, אך אין יותר מקום פנוי במהדורות: ${reservation.failedEditionIds.join(", ")} — נדרש טיפול ידני (החזר חלקי/מהדורה חלופית).`;
    await prisma.order.update({
      where: { id: order.id },
      data: {
