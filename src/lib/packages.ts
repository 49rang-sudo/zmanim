 ...
 *  משלים:   חודש = 1400 (בסיס אחיד, כמו העוגן — לא נקבע פר-חלון יותר) ·
 *           2 חודשים = 1330 לחודש (5% הנחה) · 3 חודשים = 1260 לחודש
 *           (10% הנחה) · 4+ = 10% הנחה מהרכישה השנייה והלאה.
 *
 * ---------------------------------------------------------------
 *  איך זה מתורגם לקוד
 * ---------------------------------------------------------------
 ...
 *
 *  2. עד עכשיו אצל המשלים ה"בסיס" השתנה בין חלון לחלון (1200–1350),
 *     מה שהצדיק ייצוג יחסי. הלקוחה עדכנה למחיר אחיד — 1,400 ₪,
 *     בדיוק כמו העוגן — אבל הייצוג היחסי נשאר: הוא עדיין מקור
 *     האמת התקין ביחס ל-AdSlot.priceAgorot (סעיף 1 למעלה), וגם
 *     חוסן להיסטוריה — AdSlot ישן שלא עודכן (ראו סקריפט המיגרציה,
 *     scripts/migrate-complementary-pricing.ts) עדיין יתומחר נכון
 *     ביחס למחיר שלו עצמו.
 *
...
export const TIER_PRICING: Record<PresenceTier, TierPricing> = {
  ...
  COMPLEMENTARY: {
    // 1400 · 1330 · 1260  →  1 · 0.95 · 0.9 — אותו מבנה בדיוק כמו
    // העוגן: שלוש המדרגות המפורשות שהלקוחה מסרה, ורק מעבר להן
    // (4+) נכנס כלל הזנב הכללי למטה.
    perMonthRate: [1, 0.95, 0.9],
    tailDiscount: 0.1,
    tailLabel: "10% הנחה מהחודש השני והלאה",
  },
};
 ...
 * וזה קורה עכשיו בשני הכיוונים: בעוגן (ראו האנומליה למטה) וגם
 * במשלים, מאז שקיבל אותו מבנה מדרגות בדיוק (1400·1330·1260). בכל
 * מקום שבו המחירון כבר מונוטוני מעבר לסולם המפורש, זו no-op.
 */
 ...
 * בכל חודש (למשל אחרי מיגרציה חלקית של מחירי עבר). מכפילים את הסכום
 ...
 * לא מומצאים כאן מחדש. עוגן = 1,600 ₪, משלים = 1,400 ₪ — שני המספרים
 * שהלקוחה מסרה לחודש בודד. עד לעדכון האחרון משלים היה נקבע פר-חלון
 * בטווח 1,200–1,350 ₪; הלקוחה עברה לבסיס אחיד, באותה צורה בדיוק
 * כמו העוגן (ראו ההערה הראשית למעלה וסקריפט המיגרציה למחירי עבר).
 */
export const DEFAULT_TIER_BASE_PRICE_AGOROT: Record<PresenceTier, number> = {
  ANCHOR: 160_000,
  COMPLEMENTARY: 140_000,
};
